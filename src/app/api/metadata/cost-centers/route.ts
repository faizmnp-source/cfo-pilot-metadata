...
export async function GET(req: NextRequest) {
  ...
  const where: Record<string, unknown> = {
    ...
    ...(p.search && { OR: [{ costCenterCode: { contains: p.search, mode: "insensitive" } }, { costCenterName: { contains: p.search, mode: "insensitive" } }] }),
  };
  const [data, total] = await Promise.all([
    client.findMany({ where, skip: (p.page-1)*p.pageSize, take: p.pageSize,
      orderBy: { [p.sortBy]: p.sortOrder },
      include: { parent: { select: { id:true, costCenterCode:true, costCenterName:true } }, _count: { select: { children: true } } }
    }),
    client.count({ where }),
  ]);
...
export async function POST(req: NextRequest) {
  const authResult = await requireAuthAndPermission(req, "costCenter", "create");
  if (!("auth" in authResult)) return authResult;
  const { auth } = authResult;
  ...
  await writeAuditLog({ tenantId: auth.tid, tableName: "cost_centers", recordId: record.id, dimensionType: "COST_CENTER", action: "CREATE", newValue: record, userId: auth.sub, userName: auth.name, userEmail: auth.email, userRole: auth.role });
