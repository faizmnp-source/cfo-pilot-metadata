...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    prisma.iCP.count({ where: { tenantId: tid, isActive: true } }),
    prisma.project.count({ where: { tenantId: tid, isActive: true } }),
    prisma.dimensionDefinition.count({ where: { tenantId: tid, isActive: true } }),
    ...
    prisma.importJob.count({
      where: { tenantId: tid, status: "VALIDATION_FAILED" },
    }),
  ]);
